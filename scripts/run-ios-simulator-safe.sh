...
restore_ref_file() {
  SOURCE_PATH="$1"; DEST_PATH="$2"; LABEL="$3"
  git cat-file -e "$REF:$SOURCE_PATH" 2>/dev/null || fail "Recovery source missing: $SOURCE_PATH"
  TMP_SOURCE="$(mktemp /tmp/palta-restore.XXXXXX)"; git show "$REF:$SOURCE_PATH" > "$TMP_SOURCE"; mkdir -p "$(dirname "$DEST_PATH")"
  if [ -f "$DEST_PATH" ] && ! cmp -s "$TMP_SOURCE" "$DEST_PATH"; then ensure_backup_dir; SAFE_NAME="$(printf '%s' "$SOURCE_PATH" | tr '/' '_')"; cp "$DEST_PATH" "$BACKUP_DIR/$SAFE_NAME"; info "Backed up previous $LABEL to $BACKUP_DIR/$SAFE_NAME"; fi
  if [ ! -f "$DEST_PATH" ] || ! cmp -s "$TMP_SOURCE" "$DEST_PATH"; then cp "$TMP_SOURCE" "$DEST_PATH"; info "Restored verified $LABEL."; else info "$LABEL is already on the verified version."; fi
  rm -f "$TMP_SOURCE"
}
...
# Product surfaces are authored in mobile-overlay. Sync the current checked-out branch
# into the local Expo runtime every time the simulator starts so UI work is visible
# without a second manual copy step. The sync script backs up replaced files.
if [ -x "$ROOT/scripts/sync-mobile-home-overlay.sh" ]; then
  info "Syncing current Home and Community product surfaces into the Expo runtime..."
  "$ROOT/scripts/sync-mobile-home-overlay.sh"
else
  fail "Product-surface sync script is missing or not executable: scripts/sync-mobile-home-overlay.sh"
fi

EXPERIMENTAL_STYLE="$APP_DIR/src/components/map/paltaDevelopmentMapStyle.ts"
if [ -f "$EXPERIMENTAL_STYLE" ]; then ensure_backup_dir; cp "$EXPERIMENTAL_STYLE" "$BACKUP_DIR/paltaDevelopmentMapStyle.ts"; rm -f "$EXPERIMENTAL_STYLE"; info "Removed failed experimental PMTiles style from local app."; fi

SMOKE_PATH="dev/mock-api/smoke.mjs"
git cat-file -e "$REF:$SMOKE_PATH" 2>/dev/null || fail "Recovery smoke test missing: $SMOKE_PATH"
TMP_SMOKE="$(mktemp /tmp/palta-smoke.XXXXXX.mjs)"; trap 'rm -f "$TMP_SMOKE"' EXIT; git show "$REF:$SMOKE_PATH" > "$TMP_SMOKE"
...
if ! node "$TMP_SMOKE"; then echo "--- mock API log ---" >&2; tail -80 /tmp/palta-mock-api.log 2>/dev/null || true; fail "Mock API smoke test failed."; fi
info "Mock API smoke test passed."

DEVELOPER_DIR="$(xcode-select -p 2>/dev/null || true)"; SIMULATOR_APP="${DEVELOPER_DIR}/Applications/Simulator.app"
if [ -d "$SIMULATOR_APP" ]; then info "Opening Simulator from Xcode developer directory..."; open "$SIMULATOR_APP" >/dev/null 2>&1 || true; else info "Simulator.app not found by path; continuing with simctl boot."; fi
BOOTED_UDID="$(xcrun simctl list devices booted | awk -F '[()]' '/iPhone/ && /Booted/ {print $2; exit}')"
if [ -n "$BOOTED_UDID" ]; then UDID="$BOOTED_UDID"; info "Reusing booted iPhone Simulator: $UDID"; else DEVICE_LINE="$(xcrun simctl list devices available | awk '/iPhone 18 Pro/ {print; exit}')"; [ -n "$DEVICE_LINE" ] || DEVICE_LINE="$(xcrun simctl list devices available | awk '/iPhone/ {print; exit}')"; [ -n "$DEVICE_LINE" ] || fail "No available iPhone Simulator device found in Xcode."; UDID="$(printf '%s\n' "$DEVICE_LINE" | sed -E 's/.*\(([0-9A-Fa-f-]{36})\).*/\1/')"; [ -n "$UDID" ] && [ "$UDID" != "$DEVICE_LINE" ] || fail "Could not parse Simulator UDID from: $DEVICE_LINE"; info "Booting iPhone Simulator: $DEVICE_LINE"; xcrun simctl boot "$UDID" 2>/dev/null || true; fi
xcrun simctl bootstatus "$UDID" -b
cd "$APP_DIR"
if [ ! -d node_modules ]; then info "Mobile dependencies are missing; installing them once..."; npm install; fi
info "Building, installing, and launching Palta with current product surfaces..."
