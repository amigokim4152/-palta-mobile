...
export function scheduledEventsToHome(input: {
  ...
  for (const event of input.events) {
    ...
    const needsAttentionNow = Boolean(event.actionRequired && until <= attentionLeadMs && until >= -60 * 60 * 1000);
  ...
  return {
    ...
    items: items.sort((a, b) => Date.parse(a.scheduled_at ?? '') - Date.parse(b.scheduled_at ?? '')),
  };
}
