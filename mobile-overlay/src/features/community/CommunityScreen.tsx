  ...
  const visibleFeed = useMemo(
    () =>
      !data
        ...
        : filter === 'all'
          ? data.feed
          : data.feed.filter((item) => item.kind === filter),
    [data, filter],
  );
    ...
    <ScreenFrame title="Comunidad" subtitle="Lo que está pasando en tus comunidades">
      <View style={{ gap: paltaTheme.spacing.lg }}>
        ...
        >
          ...
          <FilterChip
            label="Iglesia"
            selected={filter === 'church'}
            onPress={() => setFilter('church')}
          />
        </View>

        {!data && !error ? (
          <Text style={{ color: paltaTheme.color.textSecondary }}>Cargando comunidad…</Text>
        ) : null}
        ...
        {data ? (
          <>
            <View style={{ gap: paltaTheme.spacing.sm }}>
              <Text style={{ fontSize: 18, fontWeight: '700', color: paltaTheme.color.textPrimary }}>
                Mis comunidades
              </Text>
              {data.communities.length === 0 ? (
                <Text style={{ color: paltaTheme.color.textSecondary }}>
                  Todavía no te has unido a una comunidad.
                </Text>
              ) : (
                  ...
                  {data.communities.map((community) => (
                    ...
                    >
                      ...
                      {community.unreadCount > 0 || community.actionRequiredCount > 0 ? (
                        ...
                        >
                          {community.unreadCount > 0 ? `${community.unreadCount} nuevos` : ''}
                          {community.unreadCount > 0 && community.actionRequiredCount > 0 ? ' · ' : ''}
                          {community.actionRequiredCount > 0
                            ? `${community.actionRequiredCount} pendiente`
                            : ''}
                        </Text>
                      ) : null}
                    </Pressable>
            ...
            <View style={{ gap: paltaTheme.spacing.sm }}>
              ...
              {data.discover.map((community) => (
                <Pressable
                  key={community.id}
                  accessibilityRole="button"
                  accessibilityLabel={`Descubrir ${community.name}`}
                  onPress={() => router.push(`/community/${community.id}`)}
                  style={{
                    minHeight: paltaTheme.touch.minimum,
              ...
              {visibleFeed.length === 0 ? (
                ...
                visibleFeed.map((item) => (
                  <Pressable
                    key={item.id}
                    accessibilityRole="button"
                    accessibilityLabel={`Publicación de ${item.communityName}`}
                    onPress={() => router.push(`/community/${item.communityId}/post/${item.id}`)}
                    style={{
                      minHeight: paltaTheme.touch.minimum,
                  ...
                  >
                    ...
                    <View style={{ flexDirection: 'row', gap: 16, marginTop: 10 }}>
                      <Text style={{ fontSize: 13, color: paltaTheme.color.textSecondary }}>
                        Comentarios {item.commentCount}
                      </Text>
                      <Text style={{ fontSize: 13, color: paltaTheme.color.textSecondary }}>
                        Reacciones {item.reactionCount}
                      </Text>
                    </View>
                  </Pressable>
                ))
