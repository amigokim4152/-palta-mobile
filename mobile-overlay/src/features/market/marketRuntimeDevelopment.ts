...
type ExtendedDiscoverQuery = DiscoverMarketListingsQuery & {
  vertical?: RuntimeCompatibleVertical;
  areaRef?: string;
  maxDistanceKm?: number;
};

type ExtendedCreateCommand = CreateMarketListingCommand & {
  vertical?: RuntimeCompatibleVertical;
...
const read: MarketReadPort = {
  async discover(query) {
    const extended = query as ExtendedDiscoverQuery;
    validateExtendedDiscoveryQuery(extended);
    ...
    let items = allRecords()
      ...
      .filter((listing) => {
        ...
        if (extended.areaRef && listing.location.areaRef !== extended.areaRef) return false;
        if (
          extended.maxDistanceKm !== undefined &&
          (listing.distanceKm === undefined || listing.distanceKm > extended.maxDistanceKm)
        ) {
          return false;
        }
        if (
...
const mutation: MarketMutationPort = {
  async createListing(command: CreateMarketListingCommand) {
    const extended = command as ExtendedCreateCommand;
    ...
    createdListingVerticals.set(id, extended.vertical ?? 'secondhand');
