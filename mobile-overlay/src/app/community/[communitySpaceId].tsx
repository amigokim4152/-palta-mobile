...
export default function CommunitySpaceScreen() {
  ...
  if (state.status === 'loading' && !state.data) {
    return (
      <ScreenFrame title="Comunidad">
        <LoadingState label="Cargando comunidad…" />
      </ScreenFrame>
    );
  }
      ...
      <View style={{ gap: paltaTheme.spacing.md }}>
        {space.membershipState !== 'active' ? (
          ...
          >
            ...
            <Text style={{ marginTop: 4, color: paltaTheme.color.textSecondary }}>
              {space.joinDescription}
            </Text>
            {space.canJoin ? (
              <PaltaButton
                label={space.joinActionLabel}
                onPress={() => void communityRuntime.joinSpace(space.id).then(refresh)}
              />
            ) : null}
          </View>
        ) : null}
        ...
        {space.posts.map((post) => (
          ...
          >
            ...
            <Text style={{ marginTop: 10, fontSize: 13, color: paltaTheme.color.textSecondary }}>
              Comentarios {post.commentCount} · Reacciones {post.reactionCount}
            </Text>
          </Pressable>
        ))}
      </View>
