...
function privacyCopy(scope: string | undefined): string | null {
  if (scope === 'member_group' || scope === 'private_relation') {
    return 'Espacio privado. El contenido y los datos de relación solo se muestran a miembros autorizados.';
  }
  if (scope === 'verified_local') {
    return 'Espacio local. Palta evita publicar datos personales precisos en superficies abiertas.';
  }
  return null;
}
...
export default function CommunitySpaceScreen() {
  ...
  const acknowledge = async (postId: string) => {
    ...
    try {
      await communityRuntime.acknowledgePost(space.id, postId);
      await refresh();
    } finally {
      ...
      <View style={{ gap: paltaTheme.spacing.xl }}>
        {privacy ? (
          <View style={{ padding: paltaTheme.spacing.sm, borderRadius: paltaTheme.radius.surface, backgroundColor: paltaTheme.color.surfaceMuted }}>
            <Text style={{ fontSize: 13, lineHeight: 19, color: paltaTheme.color.textSecondary }}>{privacy}</Text>
          </View>
        ) : null}
        ...
        {space.membershipState !== 'active' ? (
          <View style={{ padding: paltaTheme.spacing.md, borderRadius: paltaTheme.radius.surface, backgroundColor: paltaTheme.color.brandSoft }}>
            ...
            {space.canJoin ? (
              <View style={{ marginTop: paltaTheme.spacing.sm }}>
                <PaltaButton label={joining ? 'Enviando…' : space.joinActionLabel} onPress={() => void join()} disabled={joining} />
              </View>
            ) : null}
          </View>
        ) : (
        ...
        {space.membershipState === 'active' && schoolItems.length > 0 ? (
          <View style={{ gap: paltaTheme.spacing.sm }}>
            <View style={{ flexDirection: 'row', justifyContent: 'space-between', gap: paltaTheme.spacing.md, alignItems: 'flex-end' }}>
              ...
              <Text style={{ fontSize: 13, fontWeight: '700', color: progress.pendingActionCount > 0 ? paltaTheme.color.brandPrimary : paltaTheme.color.textMuted }}>
            ...
            {schoolItems.map((item) => (
              <View key={item.id} style={{ paddingVertical: paltaTheme.spacing.sm, borderBottomWidth: 1, borderBottomColor: paltaTheme.color.divider }}>
                <Pressable
                  ...
                  onPress={() => router.push(`/community/${space.id}/post/${item.postId}`)}
                ...
                {item.actionRequired && item.status === 'pending' ? (
                  <Pressable
                    accessibilityRole="button"
                    accessibilityLabel={`Confirmar ${item.title}`}
                    disabled={acknowledging === item.postId}
                    onPress={() => void acknowledge(item.postId)}
                    style={({ pressed }) => ({
                  ...
                  >
                    <Text style={{ color: paltaTheme.color.brandPrimary, fontWeight: '700' }}>
                      {acknowledging === item.postId ? 'Confirmando…' : 'Confirmar'}
                    </Text>
                  </Pressable>
                ) : null}
              </View>
            ...
            <View style={{ paddingTop: 2 }}>
              <Text style={{ fontSize: 12, lineHeight: 18, color: paltaTheme.color.textMuted }}>
                WhatsApp sigue siendo útil para conversar rápido. Palta organiza aquí avisos, fechas y pendientes que conviene volver a encontrar.
              </Text>
            </View>
          </View>
        ) : null}
        ...
        <View>
          ...
          {filteredPosts.length === 0 ? (
            <View style={{ paddingVertical: paltaTheme.spacing.lg }}>
              ...
              <Text style={{ marginTop: 5, lineHeight: 20, color: paltaTheme.color.textSecondary }}>
                Los avisos y conversaciones que tengas permiso para ver aparecerán aquí.
              </Text>
            </View>
