...
assert(
  membershipStateForJoin({ joinPolicy: 'open' }) === 'active',
  'Open Community spaces should activate immediately.',
);
assert(
  membershipStateForJoin({ joinPolicy: 'approval_required' }) === 'pending',
  'Private approval spaces must create pending membership.',
);
assert(
...
assertThrows(
  () => membershipStateForJoin({ joinPolicy: 'approval_required', currentState: 'removed' }),
  'Removed members must not silently rejoin',
  'COMMUNITY_MEMBERSHIP_BLOCKED',
);
...
assertThrows(
  () => transitionCommunityMembership({
    ...
    decision: 'approve',
    assignedRoleKey: 'guardian',
    nowIso: '2026-09-18T17:13:00-03:00',
  }),
  'Active membership cannot be approved again',
  'COMMUNITY_MEMBERSHIP_INVALID_TRANSITION',
);
assertThrows(
  () => transitionCommunityMembership({
    currentState: 'pending',
    ...
    decision: 'approve',
    assignedRoleKey: 'guardian',
    nowIso: '2026-09-18T17:14:00-03:00',
  }),
  'Non-manager must not approve a request',
  'COMMUNITY_MEMBERSHIP_MANAGE_FORBIDDEN',
);
assertThrows(
  () => transitionCommunityMembership({
    currentState: 'pending',
    ...
    actorRoleKey: 'admin',
    decision: 'approve',
    nowIso: '2026-09-18T17:15:00-03:00',
  }),
  'Approval must assign a role',
  'COMMUNITY_MEMBERSHIP_ROLE_REQUIRED',
);
