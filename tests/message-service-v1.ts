...
class FakePersistence implements MessagePersistencePort {
  ...
  async transaction<T>(run: (tx: MessagePersistenceTransaction) => Promise<T>): Promise<T> {
    const snapshot = {
      conversation: { ...this.conversation },
      participants: this.participants.map((item) => ({ ...item, actor: { ...item.actor } })),
    ...
    const tx: MessagePersistenceTransaction = {
      ...
      lockConversation: async (conversationId) =>
        this.conversation.conversationId === conversationId ? { ...this.conversation } : null,
      ...
      findScope: async (scopeId) => this.scopes.find((scope) => scope.scopeId === scopeId) ?? null,
      insertMessage: async (message) => { this.messages.push(message); },
      insertAttachments: async (attachments) => { this.attachments.push(...attachments); },
      ...
      advanceRead: async (input) => {
        ...
        const next: ParticipantState = {
          ...current,
          lastReadSequence: Math.max(current.lastReadSequence, capped),
          lastDeliveredSequence: Math.max(current.lastDeliveredSequence, capped),
        };
  ...
  async listAfter(input: {
    ...
    return this.messages
      .filter((message) => message.conversationId === input.conversationId && message.sequence > input.afterSequence)
...
const authorization: MessageActorAuthorizationPort = {
  canActAs: async ({ principalUserId, actor }) =>
    actor.actorType === 'business' && principalUserId === 'staff-allowed',
};
...
assert(first.message.sequence === 1 && !first.replayed, 'First send must allocate sequence 1.');
...
assert(replay.replayed && replay.message.messageId === first.message.messageId, 'Offline retry must replay canonical message.');
assert(Number(persistence.messages.length) === 1 && Number(persistence.outbox.length) === 1, 'Retry must not duplicate durable work.');
...
await expectCode('ACTOR_NOT_AUTHORIZED', () =>
  service.send({
    ...
    sender: { actorId: 'business-1', actorType: 'business', principalUserId: 'staff-denied' },
    type: 'text',
    body: 'No autorizado',
...
assert(businessReply.message.sender.principalUserId === 'staff-allowed', 'Audit trail must preserve actual staff principal.');
...
await expectCode('ATTACHMENT_NOT_AUTHORIZED', () =>
  service.send({
    ...
    sender: userActor,
    type: 'voice',
    attachments: [{ assetId: 'asset-foreign-voice-1', kind: 'voice', mimeType: 'audio/ogg', durationMs: 4200 }],
    createdAt: '2026-09-17T19:03:20.000Z',
  }),
);
assert(Number(persistence.attachments.length) === 0, 'Unauthorized asset must not create attachment rows.');
...
assert(voice.message.attachments?.[0]?.assetId === 'asset-owned-voice-1', 'Voice must reference provider-neutral asset ID.');
assert(Number(persistence.attachments.length) === 1, 'Authorized voice attachment must persist transactionally.');
assert(!JSON.stringify(voice.outboxEvent).includes('asset-owned-voice-1'), 'Outbox must not copy attachment identifiers or payload.');
...
try {
  await service.send({
    ...
    sender: userActor,
    type: 'voice',
    attachments: [{ assetId: 'asset-owned-voice-rollback', kind: 'voice', mimeType: 'audio/ogg', durationMs: 1000 }],
...
assert(persistence.messages.length === beforeFailure.messageCount, 'Outbox failure must roll back message.');
assert(persistence.attachments.length === beforeFailure.attachmentCount, 'Outbox failure must roll back attachments.');
assert(persistence.outbox.length === beforeFailure.outboxCount, 'Outbox failure must leave outbox unchanged.');
assert(persistence.conversation.lastSequence === beforeFailure.lastSequence, 'Outbox failure must roll back sequence.');
...
assert(Number(afterSequenceOne.length) === 2, 'Cursor sync must include business reply and voice message.');
assert(afterSequenceOne[0]?.messageId === businessReply.message.messageId, 'Cursor sync must preserve order.');
assert(afterSequenceOne[1]?.messageId === voice.message.messageId, 'Cursor sync must include voice message.');
...
assert(read.lastReadSequence === persistence.conversation.lastSequence, 'Read cursor must cap at canonical last sequence.');
assert(persistence.outbox.length === outboxBeforeRead + 1, 'Forward read must emit one outbox event.');
...
assert(persistence.outbox.length === outboxAfterRead, 'Non-advancing read must not emit duplicate work.');
